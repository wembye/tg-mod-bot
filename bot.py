...
if WEB_URL and not WEB_URL.startswith("https://"):
...
async def settings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if WEB_URL:
        # Open WebApp panel
        url = f"{WEB_URL}/settings?chat_id={chat_id}"
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "⚙️ Открыть настройки" if get_settings(chat_id).get("lang","ru") == "ru" else "⚙️ Open Settings",
                web_app=WebAppInfo(url=url)
            )
        ]])
        await update.message.reply_text(
            "👆 Нажми кнопку чтобы открыть панель настроек." if get_settings(chat_id).get("lang","ru") == "ru" else "👆 Press the button to open the settings panel.",
            reply_markup=kb
        )
    else:
...
async def handle_settings_post(request):
    try:
        ...
        allowed = ["lang", "profanity_filter", "antiflood", "captcha", "block_links",
                   "mute_spam", "mute_flood", "mute_words", "custom_words"]
        for k in allowed:
...
async def handle_static(request):
    filename = request.match_info.get("filename", "settings.html")
...
async def handle_settings_page(request):
    filepath = os.path.join(STATIC_DIR, "settings.html")
    return web.FileResponse(filepath)
...
async def main_async():
    ...
    tg_app.add_handler(PrefixHandler("!", "start", start))
    tg_app.add_handler(PrefixHandler("!", "warn", warn_user))
    tg_app.add_handler(PrefixHandler("!", "ban", ban_user))
    tg_app.add_handler(PrefixHandler("!", "unban", unban_user))
    tg_app.add_handler(PrefixHandler("!", "mute", mute_user))
    tg_app.add_handler(PrefixHandler("!", "unmute", unmute_user))
    tg_app.add_handler(PrefixHandler("!", "warns", check_warns))
    tg_app.add_handler(PrefixHandler("!", "stats", stats))
    tg_app.add_handler(PrefixHandler("!", "settings", settings_cmd))
    tg_app.add_handler(PrefixHandler("!", "addword", add_word))
    tg_app.add_handler(PrefixHandler("!", "delword", del_word))
    tg_app.add_handler(PrefixHandler("!", "words", list_words))
    tg_app.add_handler(CommandHandler("start", start))

    tg_app.add_handler(CallbackQueryHandler(lang_callback, pattern="^lang_"))
    tg_app.add_handler(CallbackQueryHandler(captcha_callback, pattern="^captcha_"))
    tg_app.add_handler(CallbackQueryHandler(settings_callback, pattern="^(set_|mutecfg_|setmute_|set_links_)"))

    tg_app.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, new_member))
    tg_app.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, auto_moderate))
